...
#include <random>
#include <ctime>
void gaussSeidel(int N,double **A, double *b, double *xNew, double *xOld,double **C,double *dInv,int i);
using namespace std;
int main()
{
...
// initalize vector d inverse (1/diagonal of matrice A)
   ...
   // cout << "D inverse: " << dInv[i] << endl;
    ...
    int i=1;
    gaussSeidel(N,A,b,xNew,xOld,C,dInv,i);
    /*
    for(int i=0;i<N;i++)
    {
        cout << xOld[i] << endl;
...
void gaussSeidel(int N,double **A, double *b, double *xNew, double *xOld,double **C,double *dInv,int i)
{
   ...
   for(int row = 0; row<N; row++)
    ...
    { 
        ...
        xNew[row] = dInv[row] * (b[row] - sumOne-sumTwo);    
    } 
